...
## Real-World use case: Small Area Estimation {.smaller}

-   As part of a small area estimation workflow, we needed to extract zonal statistics from geospatial covariates for use in modelling poverty estimates at granular levels.
...
-   Processing 6 such files alongside an additional 20 raster files for other variables posed significant computational challenges. And this is what led me to investigate way to process huge raster files within limited resources.
...
- Dask enables parallel, out-of-core computation on datasets larger than RAM. 
- What you get with Dask.
  - Out-of-core computation
  - Automatic parallelization. 
  - Task scheduling: determines optimal execution order, managing dependencies between tasks, and distributing work across workers. 
  - Progress monitoring.    
...
```
Full raster: 20,000 × 13,000 pixels Chunked into:    
~260 blocks of 1024×1024 pixels each Each chunk:    
~4 MB (for float32 data) Total: Still 20,000 × 13,000, but processed in pieces   
...
::: {.callout-warning}
## Too small chunks 
- Massive task graph (potentially 100+MBs)   
...
##
### 3. Understanding Dask graphs {.smaller}
...
```         
## Dask graph best practises {.smaller}
- Use persist to reset graph. 
- Monitor your graph size.  
- Choose the right Dask API. 
  - For instance map_blocks vs delayed. 
  - map_blocks failed because it expects array-like outputs whereas my function returns dictionaries so metadata inference fails. 
  - delayed worked as its flexible for arbitrary Python objects and it doesn't require metadata. 

## 4. Reprojection with WarpedVRT {.smaller}
...
# Problem: Chunks misalign across CRS transformations
Source chunks (UTM):        Dest chunks (WGS84):
┌────┬────┐                ┌────┬────┐
│ A  │ B  │  Reproject →   │ 1  │ 2  │
├────┼────┤                ├────┼────┤
│ C  │ D  │                │ 3  │ 4  │
...
Chunk 1 needs data from: A + B + C  ← Data shuffling!
Chunk 2 needs data from: B + D      ← Redundant reads!

```         
## What is WarpedVRT? {.smaller}
- This is a virtual raster format that performs on-demand, windowed reprojection.   
- How it works:   
...
# VRT is now a "virtual file" that appears reprojected   
# but hasn't actually transformed anything yet!          

raster = rioxarray.open_rasterio(vrt, chunks={"x": 1024, "y": 1024})   
# Data stays lazy, chunks aligned to destination grid   
```
         
## When you read a chunk {.smaller}
- VRT computes source window (UTM Zone 46S) EPSG:4326 (35.5 degrees, -12.0 degrees) -> UTM (450000M, 8670000M).
- So it reads only that source window from disk. 
- Apply resampling on-the-fly (sum/neares/bilinear)
- Return transformed pixels in destination grid 

## My implementation {.smaller}

```python

def reproject_to_basegrid_vrt( src_raster_path, base_crs="EPSG:4326", base_res=(0.00083333333, -0.00083333333), resampling_method=Resampling.sum, out_chunks={"x": 1024, "y": 1024}, ): with rasterio.open(src_raster_path) as src: 
# Calculate destination grid left, bottom, right, top = src.bounds xres, yres = base_res dst_transform = from_origin(left, top, xres, abs(yres)) dst_width = int(round((right - left) / xres)) dst_height = int(round((top - bottom) / abs(yres)))
         
    # Create virtual reprojected raster
    vrt = WarpedVRT(
        src,
        crs=base_crs,
        transform=dst_transform,
    ...
    return da_vrt  # Lazy, chunked, virtually reprojected!
...
- map_blocks for array transformations.
- delayed for custom aggregations.

4.The journey is iterative.  
